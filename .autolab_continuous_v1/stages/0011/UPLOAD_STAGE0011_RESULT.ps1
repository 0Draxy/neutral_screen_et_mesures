...
$Log=Join-Path $DevRoot "logs\AUTOLAB_UPLOAD_STAGE0011.log"
...
$token="AUTOLAB0011-"+$sha.Substring(0,10)+"-"+(Get-Date -Format "HHmmss")
...
@(
    ...
    "sha256=$sha"
    "token=$token"
    "engine=V18_V17_FROZEN_GENERIC_STAGE0011"
)|Set-Content $MarkerPath -Encoding UTF8

Log "UPLOAD_CONFIRMED STAGE0011"
exit 0
