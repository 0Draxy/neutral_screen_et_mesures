...
def test_scpi_closed_logic() -> None:
    ...
    result = good.safe_stop_and_confirm(settle_s=0, attempts=1)
    assert result["confirmed"] is True, result
    assert "OUTP OFF" in good.ser.commands
    assert "VOLT 0" in good.ser.commands
    ...
    result = empty_error.safe_stop_and_confirm(settle_s=0, attempts=1)
    assert result["confirmed"] is False
    assert any("file SCPI" in err for err in result["errors"])
    ...
    result = empty_state.safe_stop_and_confirm(settle_s=0, attempts=1)
    assert result["confirmed"] is False
    assert any("état générateur" in err for err in result["errors"])
    ...
    result = too_high.safe_stop_and_confirm(settle_s=0, attempts=1)
    assert result["confirmed"] is False
    assert any("tension résiduelle" in err for err in result["errors"])

    still_running = make_ea({
        "FUNC:GEN:WAVE:STAT?": "RUN",
        "OUTP?": "0",
        "MEAS:VOLT?": "0.015",
        "SYST:ERR?": '+0,"No error"',
    })
    result = still_running.safe_stop_and_confirm(settle_s=0, attempts=1)
    assert result["confirmed"] is False
    assert any("générateur non arrêté" in err for err in result["errors"])

    assert appmod.EAPSU.generator_state_is_running("RUN")
    assert appmod.EAPSU.generator_state_is_running("RUNNING")
    assert appmod.EAPSU.generator_state_is_running("1")
    assert not appmod.EAPSU.generator_state_is_running("")
...
def test_sqlite_save(ihm: appmod.IhmRelaisRp2040, base: Path) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        shutil.copy2(base / "chronometrie_contacts.sqlite3", db)
