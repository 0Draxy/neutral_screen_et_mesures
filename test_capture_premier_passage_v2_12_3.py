...
def test_pickup_bounce() -> None:
    s = CaptureState()
    stable_us = 3000
    s.update(12_000_000, True, 12_000, stable_us)   # premier passage travail
    s.update(12_001_000, False, 12_001, stable_us)  # rebond
    s.update(12_030_000, True, 12_030, stable_us)   # retour travail
    s.update(12_033_000, True, 12_033, stable_us)   # validation stable
    assert s.done
...
def test_dropout_bounce() -> None:
    s = CaptureState()
    stable_us = 3000
    s.update(7_980_000, True, 12_020, stable_us)    # premier passage repos
    s.update(7_981_000, False, 12_019, stable_us)   # rebond
    s.update(8_010_000, True, 11_990, stable_us)    # retour repos
    s.update(8_013_000, True, 11_987, stable_us)    # validation stable
    assert s.done
