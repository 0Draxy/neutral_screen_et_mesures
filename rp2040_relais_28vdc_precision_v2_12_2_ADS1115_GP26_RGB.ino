/*
  RP2040 - Pilotage relais 28 VDC - V2.12.2 selection tension Neutral screen + LED RGB etats IHM

  Etat fonctionnel V2.12.2 :
  ...
  - Trame HELLO annonce V2_12_2 a l'IHM.
  ...
  Commandes serie :
    ...
    PULSE_US;BE;DUREE_US      -> selection tension haute puis pulse BE
    PULSE_US;BR;DUREE_US      -> selection tension haute puis pulse BR
    PULSE_US;BEBR;DUREE_US    -> selection tension basse puis pulse BE+BR
    MEASURE_CONTACTS;BE;CAPTURE_US;PULSE_US;NB_INV  -> chronometrie contacts BE
    MEASURE_CONTACTS;BR;CAPTURE_US;PULSE_US;NB_INV  -> chronometrie contacts BR
    MEASURE_MONO;ON;CAPTURE_US;HOLD_US;NB_INV       -> chronometrie monostable enclenchement GP14
    MEASURE_MONO;OFF;CAPTURE_US;HOLD_US;NB_INV      -> chronometrie monostable declenchement GP14
...
void traiterMeasureContacts(const String &cmd) {
  ...
  // La selection voie haute est preparee avant t0. Le temps mesure commence
  // au front de commande bobine BE/BR, pas au collage du relais de selection.
...
void traiterPulseUs(const String &cmd) {
  ...
  if (typeTxt == "BE") {
    ...
    demarrerPulseNeutral(0x01, dureeUs, "BE", true);
  } else if (typeTxt == "BR") {
    ledRgbSetMode(LED_MODE_PULSE_BR);
    demarrerPulseNeutral(0x02, dureeUs, "BR", true);
...
void setup() {
  ...
  Serial.println("RP2040_RELAIS_28VDC_PRET;V2_12_2;SELECTION_TENSION_BASSE_HAUTE_INFO_GP26;LED_RGB_GP16;LED_COMMANDS;MEASURE_CONTACTS;MEASURE_MONO;ADS1115_GP0_GP1;VOLTAGE_MONO_BISTABLE;CALIBRATION_RAW;VOLTAGE_FIRST_PASSAGE;STABLE_CONFIRMATION;CAPTURE_QUALITY_50US");
  envoyerStatus("BOOT");
}
